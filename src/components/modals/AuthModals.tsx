...
export const ForgotPasswordModal: React.FC<ForgotPasswordModalProps> = ({ onClose, onOpenReset }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      showToast({ type: 'success', title: 'تم الإرسال', message: 'تم إرسال تعليمات إعادة التعيين إلى بريدك الإلكتروني' });
    } catch (err: any) {
      const msg = err.message || 'حدث خطأ، يرجى المحاولة لاحقاً';
      setError(msg);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 dir-rtl">
      <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-5 border border-slate-200 shadow-2xl relative">
        <button onClick={onClose} className="absolute top-5 left-5 p-1.5 bg-slate-100 hover:bg-slate-200 rounded-full text-slate-500">
          <X className="w-5 h-5" />
        </button>
        ...
        {success ? (
          <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-200 space-y-3 text-center">
            ...
            <p className="text-xs font-extrabold text-emerald-950">تم إرسال تعليمات إعادة التعيين إلى بريدك الإلكتروني</p>
            <p className="text-[11px] text-emerald-800">يرجى فحص صندوق الوارد والنقر على الرابط لتعيين كلمة مرور جديدة.</p>
            <button onClick={onClose} className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-black shadow-xs">
...
export const ResetPasswordModal: React.FC<ResetPasswordModalProps> = ({ email, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'تم التحديث', message: 'تم تحديث كلمة المرور بنجاح!' });
      onSuccess();
      onClose();
    } catch (err: any) {
      const msg = err.message || 'فشل تحديث كلمة المرور، يرجى المحاولة لاحقاً';
      setError(msg);
      showToast({ type: 'error', title: 'فشل التحديث', message: msg });
    } finally {
      setIsLoading(false);
    }
  };
