...
export default function FinanceAdminDashboardView() {
  ...
  const loadFinanceDataDirectly = async (showBadge = true) => {
    ...
    try {
      ...
      const msg = 'تعذر تحميل أحدث البيانات المالية من خادم Supabase.';
      setError(msg);
      showToast({ type: 'error', title: 'فشل التحميل', message: msg });
    } finally {
      setLoading(false);
  ...
  const handleApprovePayout = (payoutId: string, amount: number, recipientName: string) => {
    showConfirm({
      title: 'تأكيد الصرف',
      message: `هل أنت متأكد من صرف مبلغ ${formatCurrency(amount)} للمستفيد "${recipientName}"؟`,
      variant: 'info',
      confirmLabel: 'موافقة وصرف',
      onConfirm: async () => {
        try {
          await StorageRepo.updatePayoutStatus(payoutId, 'completed');
          showToastMsg('تمت موافقة المحاسب المالي وتمرير السحب بنجاح');
        } catch (err: any) {
          showToastMsg(`تعذر الموافقة على السحب: ${err.message || 'خطأ غير معروف'}`, 'error');
        }
      },
    });
  };

  const handleRejectPayout = (payoutId: string, amount: number, recipientName: string) => {
    showConfirm({
      title: 'تأكيد الرفض',
      message: `هل أنت متأكد من رفض تحويل مبلغ ${formatCurrency(amount)} للمستفيد "${recipientName}"؟`,
      variant: 'danger',
      confirmLabel: 'رفض',
      onConfirm: async () => {
        try {
          await StorageRepo.updatePayoutStatus(payoutId, 'failed');
          showToastMsg('تم رفض السحب وإعادة المبلغ لرصيد الحساب');
        } catch (err: any) {
          showToastMsg(`تعذر رفض السحب: ${err.message || 'خطأ غير معروف'}`, 'error');
        }
      },
    });
  };
  ...
  const handleUpdateStoreCommission = (storeId: string, newRate: number) => {
    ...
    if (!store) return;
    showConfirm({
      title: 'تأكيد تعديل العمولة',
      message: `هل أنت متأكد من تعديل نسبة عمولة منصة جِهَات لمتجر "${store.name}" إلى ${newRate}%؟`,
      variant: 'info',
      confirmLabel: 'تعديل',
      onConfirm: async () => {
        try {
          ...
          showToastMsg(`تم تعديل نسبة عمولة منصة جِهَات لمتجر "${store.name}" إلى ${newRate}%`);
        } catch (err: any) {
          showToastMsg(`تعذر تعديل العمولة: ${err.message || 'خطأ غير معروف'}`, 'error');
        }
      },
    });
  };
      ...
      <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-teal-950 text-white rounded-3xl p-6 sm:p-8 space-y-3 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => showToastMsg('تم تصدير التقرير المالي المعتمد بصيغة Excel/PDF')}
                        ...
                        <td className="p-3 text-center">
                          {payout.status === 'pending' ? (
                            <div className="flex items-center justify-center gap-2">
                              <button
                                onClick={() => handleApprovePayout(payout.id, payout.amount, payout.store_name || payout.user_name || '')}
                              ...
                              <button
                                onClick={() => handleRejectPayout(payout.id, payout.amount, payout.store_name || payout.user_name || '')}
